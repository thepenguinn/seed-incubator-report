...
\subtikzpicturedef{subEMCUOverview} {
    ...
    \innerEnclosureNode {#1-outerGMUBoard} {#1-tmp1} {#1-tmp2}

    \draw
    (#1-outerGMUBoard.north west) ++(\absInnerPad, -\absInnerPad) coordinate (#1-tmp1)
    ++(\absIICBoardWidth, -\absIICBoardHeight) coordinate (#1-tmp2)
    ;

    \unfinishedBoardNode {#1-tmp} {#1-tmp1} {#1-tmp2} {\texttt{ECAM}}

    \draw
    (#1-outerGMUBoard.south east)
    ++(-\absInnerPad, \absInnerPad)
    node [anchor = south east, font = \LARGE] {
        \texttt{GMU}
    }
    ...
    (#1-outerGMUBoard.south -| #1-start) ++(0, -\absOuterPad) coordinate (#1-end)
    (#1-start) -- (#1-end)
    ;
