...
Now that we have sorted out all the components, we can put all of these
together to make our \emph{serially addressable $32$ bit multiplexer}.
\texttt{QA} to \texttt{QD} are buffered and level shifted using inverters and
connected to the address pins of both the \emph{74HC4067}s. The \texttt{EN} pins are
tied together using some logic circuits to the \texttt{QE} pin. Refer figure
\ref{fig:absThirtyTwoBitMux} for a detailed circuit diagram.
...
\alertNote{
    Figure \ref{fig:absThirtyTwoBitMux} is only showing one of the multiplexer
    units. The multiplexer board consists of two such circuits. Both will be
